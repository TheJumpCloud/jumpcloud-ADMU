function Start-Migration {
    ...
    process {
        try {
            ...
            # Restore login if blocked and log the error
            if ($accountLoginBlocked) {
                Write-ToLog -Message:("Restoring interactive logon for '$SelectedUserName' (SID: $SelectedUserSID) due to error in process block.")
                $null = Set-AccountLoginPolicy -SID $SelectedUserSID -Action Enable
                $accountLoginBlocked = $false
            }
            # Log the error without re-throwing - allow graceful end block cleanup
            Write-ToLog -Message:("Error during migration process: $_") -Level Error
        }
    }
    end {
        $FixedErrors = @();
