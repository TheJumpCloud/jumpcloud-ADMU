function Write-AdmuErrorMessage {
    ...
    switch ($ErrorName) {
        ...
        "user_profile_folder_name_error" {
            Write-ToLog -Message:("User Profile Folder Name Error: The user profile folder name contains a .WORKGROUP or .ADMU suffix, which is not allowed. This indicates that the user has been migrated previously.") -Level Warning
        ...
        "user_profile_previous_sid_error" {
            Write-ToLog -Message:("User Profile Previous SID Error: The user profile has a PreviousSID key. The user has been migrated previously.") -Level Warning

            $Script:ErrorMessage = "User Profile Previously Migrated Error. Click the link below for troubleshooting information."
        }
        Default {
            Write-ToLog -Message:("Error occurred, please refer to this link for more information: https://github.com/TheJumpCloud/jumpcloud-ADMU/wiki/troubleshooting-errors") -Level Warning
