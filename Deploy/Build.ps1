...
If (-not [System.String]::IsNullOrEmpty($NewContent))
{
    ...
    {
    ps2exe -inputFile 'C:\agent\_work\1\s\Deploy\ADMU.ps1' -outputFile 'C:\agent\_work\1\s\jumpcloud-ADMU\exe\gui_jcadmu.exe' -title 'JumpCloud ADMU' -product 'JumpCloud ADMU' -description 'JumpCloud AD Migration Utility' -copyright '(c) 2021' -version $Version.Matches.Value -company 'JumpCloud' -requireAdmin -iconfile 'C:\agent\_work\1\s\Deploy\admu.ico'
